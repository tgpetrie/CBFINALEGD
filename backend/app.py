...
def sign_coinbase_request(method, request_path, body=''):
    """Create signature for authenticated Coinbase API requests"""
    timestamp = str(int(time.time()))
    message = timestamp + method + request_path
    if body:
        message += body if isinstance(body, str) else json.dumps(body)
    
    # Get the API secret (private key)
    private_key = os.environ.get('COINBASE_API_SECRET')
    
    # Check if private key is in PEM format and extract the base64 part if needed
    if '-----BEGIN EC PRIVATE KEY-----' in private_key:
        ...
        base64_content = ''.join(line for line in private_key_lines 
                               if not line.startswith('-----BEGIN') and 
        ...
        # Use as is if not in PEM format
        signature = hmac.new(
...
def coinbase_api_request(endpoint, method='GET', params=None, data=None):
    """Make a request to Coinbase API, trying public API first"""
    ...
    # Try public API first (works for most endpoints)
    try:
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'Mozilla/5.0 CBMo4ers-Dashboard/1.0'
        }
        ...
        if response.status_code == 200:
            logging.info(f"Successfully made public API request to {endpoint}")
            return response.json()
        elif response.status_code == 401 and COINBASE_API_KEY and COINBASE_API_SECRET:
            # Only try auth if we get unauthorized and have credentials
            logging.info(f"Public API returned 401 for {endpoint}, trying authenticated request")
        else:
            logging.error(f"Coinbase public API Error: {response.status_code} - {response.text}")
            return None
            
    except Exception as e:
        logging.error(f"Coinbase public API request error: {e}")
        # Fall through to try authenticated if credentials available
    
    # Fallback to authenticated API only if we have credentials and public failed
    if COINBASE_API_KEY and COINBASE_API_SECRET:
        try:
            headers = sign_coinbase_request(method, endpoint, data)
            ...
            if response.status_code == 200:
                logging.info(f"Successfully made authenticated request to {endpoint}")
                return response.json()
            else:
                logging.error(f"Authenticated request failed: {response.status_code}")
                return None
...
def setup_coinbase_websocket():
    """Setup and connect to Coinbase WebSocket for real-time updates"""
    global ws_conn, coinbase_ws_connected
    ...
    def on_message(ws, message):
        ...
        if data.get('type') == 'ticker':
            ...
            if product_id:
                symbol = product_id.split('-')[0]  # Extract BTC from BTC-USD
                price = float(data.get('price', 0))
                percent_change_24h = float(data.get('price_24h_change', 0))
                volume_24h = float(data.get('volume_24h', 0))
                volume_30d = float(data.get('volume_30d', 0))
                
                coinbase_ws_data[symbol] = {
                    "symbol": symbol,
                    "current_price": price,
                    "price_change_24h_percent": percent_change_24h,
    ...
    def on_close(ws, close_status_code, close_msg):
        ...
        threading.Timer(5.0, setup_coinbase_websocket).start()
    
    def on_open(ws):
        logging.info("WebSocket connection opened")
        coinbase_ws_connected = True
    ...
    websocket.enableTrace(CONFIG.get('DEBUG', False))
    ws_conn = websocket.WebSocketApp("wss://ws-feed.exchange.coinbase.com",
                                    on_open=on_open,
...
def get_24h_top_movers():
    ...
    try:
        # Use Coinbase as primary source since it's more reliable
        return get_coinbase_24h_top_movers()
    except Exception as e:
        logging.error(f"Coinbase 24h top movers API failed: {e}, trying CoinGecko backup...")
...
def get_coinbase_24h_top_movers():
    ...
    try:
        ...
        for product in usd_products[:50]:  # Reduced from 100 to 50 for faster loading
            try:
                # Get 24h stats
            ...
            time.sleep(0.05)  # Increased from 0.02 to 0.05 to avoid rate limits
...
def get_historical_chart_data(symbol, days=7):
    """Fetch historical price data for charts with caching"""
    global chart_cache
    
    # Check cache first
    cache_key = f"{symbol}_{days}"
    if cache_key in chart_cache:
        cache_entry = chart_cache[cache_key]
        if time.time() - cache_entry['timestamp'] < 300:  # 5 minute cache
            logging.info(f"Using cached chart data for {symbol} ({days} days)")
    ...
    try:
        ...
        # Use mapped coin ID or fallback to lowercase
        ...
        # Try using Coinbase API first for popular coins
        if symbol in ["BTC-USD", "ETH-USD", "SOL-USD", "LINK-USD", "DOGE-USD", "AVAX-USD"]:
            try:
                start_time = time.time()
                interval = "ONE_DAY" if days > 7 else "ONE_HOUR"
                cb_url = f"https://api.exchange.coinbase.com/products/{symbol}/candles"
                params = {
                    'granularity': 3600 if days <= 7 else 86400,  # hourly or daily
                    'start': datetime.now().replace(hour=0, minute=0, second=0).timestamp() - (days * 86400),
                ...
                response = requests.get(cb_url, params=params, headers=headers, timeout=8)
                ...
                if response.status_code == 200:
                    ...
                    logging.info(f"Successfully fetched {len(chart_data)} chart points from Coinbase for {symbol} in {time.time() - start_time:.2f}s")
                    return chart_data
                    
            except Exception as cb_error:
                logging.warning(f"Coinbase chart data failed for {symbol}, falling back to CoinGecko: {cb_error}")
        
        # Continue with CoinGecko as fallback
        url = f"https://api.coingecko.com/api/v3/coins/{coin_id}/market_chart"
        params = {
            'vs_currency': 'usd',
            'days': min(days, CONFIG['CHART_DAYS_LIMIT']),
        ...
        if response.status_code == 200:
            ...
            logging.info(f"Successfully fetched {len(chart_data)} chart points from CoinGecko for {symbol}")
            return chart_data
...
def get_watchlist():
    """Get top cryptos with 24hr changes and volume metrics"""
    try:
        ...
        params = {
            ...
            'per_page': 30,              # Get top 30 coins
            'page': 1,
            'sparkline': False,
            'price_change_percentage': '1h,24h'  # Get hourly and daily changes
        }
        ...
        # Format the data with focus on 24h changes and volume metrics
        formatted_data = []
        for coin in coins:
            current_price = float(coin['current_price'])
            price_change_24h = float(coin.get('price_change_percentage_24h', 0))
            ...
            # Calculate volume change (using 1h price change as an estimate for volume change)
            # In a real implementation, you'd compare current volume to 1h ago volume
            volume_change_estimate = price_change_1h * 0.8  # Simplified estimate
            ...
            formatted_data.append({
                ...
                "volume_24h": volume_24h,
                "volume_change_1h_percent": volume_change_estimate,  # Estimated volume change
                "market_cap": market_cap,
                "image": coin.get('image', '')
            })
        
        # Sort by absolute 24h price change (biggest movers first)
        formatted_data.sort(key=lambda x: abs(x["price_change_24h_percent"]), reverse=True)
...
def test_connection():
    ...
    try:
        # Make a simple request to the Coinbase API
        response = coinbase_api_request('/currencies')
        
        if response:
            return jsonify({
                "status": "success",
                "message": "Connection to Coinbase API successful",
                "data": {
                    "currencies_count": len(response)
                }
            })
        else:
            return jsonify({
        ...
        return jsonify({
            "status": "error",
            "message": f"Error connecting to Coinbase API: {str(e)}"
        }), 500
